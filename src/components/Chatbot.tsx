  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-16 h-16 bg-mint text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all duration-300 group"
      >
        {isOpen ? (
          ...
          <svg className="w-8 h-8 group-hover:animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Chat Window */}
      <div className={`fixed bottom-24 right-6 z-50 w-[350px] md:w-[400px] h-[550px] bg-white rounded-2xl shadow-3xl border border-gray-100 flex flex-col transition-all duration-500 overflow-hidden ${
        isOpen ? "translate-y-0 opacity-100 scale-100" : "translate-y-10 opacity-0 scale-95 pointer-events-none"
      }`}>
        {/* Header */}
        <div className="bg-mint p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white font-serif font-bold italic">
            M
          </div>
          <div>
            <h3 className="text-white font-serif font-bold leading-tight text-lg">MINT Assistant</h3>
            <p className="text-white/80 text-xs font-sans uppercase tracking-widest">Always Online</p>
          </div>
        </div>
